...
class DiskStorageFruit implements IStorageFruit {
    public async saveFile(file: string): Promise<string> {
        ...
        await fs.promises.rename(
            ...
            path.resolve(uploadConfig.uploadsFruit, file),
        );
    ...
    public async deleteFile(file: string): Promise<void> {
        const filePath = path.resolve(uploadConfig.uploadsFruit, file);
...
export default DiskStorageFruit;
